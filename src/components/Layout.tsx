...
const Layout = () => {
  return (
    <>
      ...
      <p className="text-white text-center text-[18px] mt-[10px]">A complete reusable component library</p>
      <hr className="mt-10 mb-5 border-tertiary" />
      <ul className="flex flex-wrap gap-4 my-12">
        {links?.map(({ id, name, link }) => (
          <li className="" key={id}>
            <NavLink to={link}>
              <span className="text-white text-[25px] border border-blue-100 border-solid w-[200px] h-[150px] flex items-center justify-center hover:bg-amber-500">
                {name}
              </span>
            </NavLink>
          </li>
